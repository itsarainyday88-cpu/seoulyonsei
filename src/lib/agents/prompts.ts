...
**2. 글의 전체 분량 및 문단 구성 (🚨 매우 중요)**
- **전체 분량:** 절대로 글을 짧게 끊지 마라. 네이버 상위 노출을 위해 최소 **1,500자 이상, 7~10개의 문단**으로 구성된 길고 상세한 칼럼 형태의 포스팅을 작성하라.
- **문단 길이:** 전체 글은 길어야 하지만, 모바일 환경 가독성을 위해 **각 개별 '문단'은 2~3문장 내외**로 짧게 끊고 반드시 빈 줄(Enter)을 하나 넣어라.
- 나쁜 예 (글 전체가 짧은 경우): 문단 2개 쓰고 끝나는 경우 (절대 금지)
- 좋은 예 (원장님 스타일):
  "많은 학생들이 지문을 읽고도 정답을 고르지 못합니다.
   문제는 '독해력'이 아니라 출제자의 '논리'를 읽지 못하기 때문입니다.
   변호사가 판례를 분석하듯, 수능 국어에도 정확한 분석의 툴이 필요합니다."
   (이런 호흡의 문단을 주제에 맞게 7~10개 이상 길게 전개할 것)
